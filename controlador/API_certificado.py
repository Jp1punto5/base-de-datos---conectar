    ...
    try:
        ...
        with get_connection_dynamic(sesion['server'],sesion['user'],sesion['password'],database) as conn:
            ...
            query = """
                        ...
                        if object_id('tempdb..#clientes') is not null begin drop table #clientes end
                        ...
                        -- insertamos los clientes actuales.
                        insert into #clientes
                        values
                        ('77225200-5','SALFA - ARRENDADORA DE VEHICULOS S.A.'),
                        ('77842840-7','GEOROCK S.A.'),
                        ('83547100-4','AUTORENTAS DEL PACIFICO S.A.'),
                        ('80914400-3','SGS CHILE LTDA'),
                        ('96801810-8','SGS MINERALS'),
                        ('77350475-k','SGS SIGA - DHSIGA SPA'),
                        ('78929230-2','SIGA INGENIERIA Y CONSULTORIA S.A.')
                        --('','') -- este es para crear mas y copiar y pegar


                        -- insertamos patente, IDGPS y Rut
                        insert into #certificado (patente,imei,rut, correo)
                        select
                        mov_codigo,
                        MOV_IDGPS,
                        MOV_RUT,
                        case
                                when mov_rut = '77225200-5' then 'sacsalfa@wisetrackcorp.com'
                                when mov_rut in 
                                                (
                                                    '80914400-3',
                                                    '96801810-8',
                                                    '77350475-k',
                                                    '78929230-2'
                                                )                   then 'juan.berrios@wisetrackcorp.com'
                                else 'paulina.villanueva@wisetrackcorp.com'
                        end 
                        ...
                        -- vamos a agregar la razón social segun el rut

                        update ce 
                        set ce.razonSocial = case
                                            when c.razonSocial is not null then c.razonSocial
                                            else 'Buscar Razon Social'
                                    end 
                        from #certificado ce
                        ...
                        update ce  
                            ...
                            select
                                ...
                                case 
                                        ...
                                        WHEN ID_TipoIntegracion = 97 THEN 'BHP - SPENCE'
                                        WHEN ID_TipoIntegracion IN (53,54) THEN 'BHP'
